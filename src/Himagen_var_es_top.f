c     Program yukawa.f
c     ******************************************************************
c                    Juan Carlos del Valle y Daniel Nader
c
c     USAGE:  ./yukawa.e < h.dat &
c
c     --- This program minimizes the energy of the Yukawa Potential:
c                                   Ground State
c       
c     --- Using MINUIT from CERN-lib and
c
c     --- D01FCF INTEGRATION SUBROUTINE  NAG-lib
c
cc     Compile as: gfortran -o yukawa.e yukawa.f
c
      PROGRAM MAIN
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c     .. COMMON BLOCK FOR INPUT DATA ...
c      COMMON / XINPUT / xB, RLN(5), RLD(5), ZLN(10), ZLD(10),
c     $ nrhoN, nrhoD, nzN, nzD, EPNUM(20), EPDEN(20), EPSin, EPSid,
c     $ FCT, icorr, EPSC
      COMMON / NINPUT3 / nrhoN, nrhoD, nzN, nzD, nakN, nakD, icorr
      COMMON / XINPUT2 / xB, EPSin, EPSid,  FCT, EPSC
      COMMON / PARHAM /  ep1,ep2, efm, im,incharge,sigma,k
      COMMON / XINPUT1 / RLN(5), RLD(5), ZLN(10), ZLD(10), EPNUM(20)
     $, EPDEN(20), AKLN(10), AKLD(10)
      COMMON / MONITOR / ebest, icall, icallb
c     EXTERNAL FUNCTION FOR MINUIT MINIMIZATION
      EXTERNAL FCN, futil
...
c
      ...
      ebest  = 1.D10
c
      read(5,*)    ! Read parameters of the Hamiltonian (Z)
      read(5,*)incharge, ep1, ep2, im, efm, sigma  
      read(5,*)
      read(5,*)k
      read(5,*)
      read(5,*)nrhoN, nrhoD  ! Number of regions in rho (NUM / DEN)
      read(5,*)
      read(5,*)(RLN(ir), ir=1,nrhoN+1)  ! rho Num limits
      read(5,*)
      read(5,*)(RLD(ir), ir=1,nrhoD+1)  ! rho Den limits
      read(5,*)
      read(5,*)nzN, nzD     ! Number of regions in z (NUM / DEN)
      read(5,*)
      read(5,*)(ZLN(ir), ir=1,nzN+1)   ! z Num limits
      read(5,*)
      read(5,*)(ZLD(ir), ir=1,nzD+1)   ! z Den limits
      read(5,*)
      read(5,*)nakN, nakD     ! Number of regions in z (NUM / DEN)
      read(5,*)
      read(5,*)(AKLN(ir), ir=1,nakN+1) ! k Num limits
      read(5,*)
      read(5,*)(AKLD(ir), ir=1,nakD+1)   ! k Den limits
      read(5,*)
      read(5,*)EPSin, EPSid, FCT, icorr, epsc, nscreen
c
       units=1.D0       ! Our units (Proper definition of Ry)      
       xB=Bx/units        
c                         ! See common DATOS
c
      if(nscreen.eq.0)then
      open(unit=6,file='minuit.out',status='unknown')
      end if
c
      write(6,*)
      write(6,*)
      write(6,*)'******************************************************'
      write(6,*)'This program minimizes the variational Energy for     '
      write(6,*)'              the H like atoms with charges (images)  '
      write(6,*)'   produced by a change of dieelectric media          '
      write(6,*)'                                                      '
      write(6,*)'******************************************************'
      write(6,*)
      write(6,*)'Parameters of the Hamiltonian'
      write(6,*)'Nuclear charge : ',incharge
      write(6,*)'epsilon 1:',ep1
      write(6,*)'epsilon 2:',ep2
      write(6,*)'magnetic quantum number : ',im
      write(6,*)'k=',k
      write(6,*)'sigma', sigma
      write(6,*)
c
        ...
        write(*,*)'--------------------------------------------------'
        write(*,*)
        write(*,*)(RLN(ir), ir=1,nrhoN+1)  ! rho Num limits
        write(*,*)
        write(*,*)(RLD(ir), ir=1,nrhoD+1)  ! rho Den limits
        write(*,*)
        write(*,*)(ZLN(ir), ir=1,nzN+1)   ! z Num limits
        write(*,*)
        write(*,*)(ZLD(ir), ir=1,nzD+1) ! z Den limits
        write(*,*)
        write(*,*)(AKLN(ir), ir=1,nakN+1) ! z Num limits
        write(*,*)
        write(*,*)(AKLD(ir), ir=1,nakD+1)   ! z Den limits
        write(*,*)
        write(*,*)'--------------------------------------------------'
        write(*,*)
        write(*,*)'EPS:', EPSin, EPSid
        write(*,*)'FACTOR:',FCT
        write(*,*)
c
c     "FLAT" DISTRIBUTION (SAME ACCURACY FOR ALL SUBINTERVALS)
        DO in= 1, 20  ! max regions = 20
           EPNUM(in) = EPSIn
           EPDEN(in) = EPSid
        END DO
c
c
 5001   format(1x,5(F10.5,2x))
 5002   format(1x,4(F10.5,2x))
 5003   format(1x,2(F10.5,2x))
c
c     -----------------------------------------------
...
c     SUBROUTINE FCN
c
      SUBROUTINE FCN(NPAR,GR,FVAL,XV,IFLAG,FUTIL)
      IMPLICIT  DOUBLE PRECISION(A-H,O-Z)
c     MINUIT parameters and common definitions
       DIMENSION GR(*),XV(*)
c     .. Parameters for the integration routine D01FCF ...
      INTEGER          NDIM,  MAXPTS,         LENWRK
      PARAMETER       (NDIM=3,MAXPTS=100000000,LENWRK=5000000)
c     .. LOCAL ARRAYS ...
      DOUBLE PRECISION ANUM(NDIM), BNUM(NDIM), ADEN(NDIM), BDEN(NDIM), 
     $ WRKSTR(LENWRK), XNU(20), XDE(20), AORT(NDIM), BORT(NDIM)    
c     .. COMMON BLOCK FOR INPUT DATA ...
c      COMMON / XINPUT / xB, RLN(5), RLD(5), ZLN(10), ZLD(10),
c     $ nrhoN, nrhoD, nzN, nzD, EPNUM(20), EPDEN(20), EPSin, EPSid,
c     $ FCT, icorr, EPSC
     ...
     $, EPDEN(20),AKLN(10), AKLD(10)
      COMMON / MONITOR / ebest, icall, icallb
c     .. COMMON BLOCK  FOR WAVE FUNCTIONS PARAMETERS ...
      COMMON / PARM / B, AA, a, h, c, d, alpha, beta, delta
      COMMON / PARM2 / z0, alfac, alfa1, alfa2, cc
      COMMON  / PARMGS / alfacgs, alfa1gs, alfa2gs
      COMMON /FLAGS / iqflag,nmax
c     .. COMMON BLOCK FOR ABBREVIATIONS ...
      COMMON / ABBR / B2, SQRB, FCTR
c     .. INTRINSIC FUNCTIONS ...
      INTRINSIC   ABS
c     .. EXTERNAL FUNCTIONS ...
      EXTERNAL          D01FCF, EVARN, EVARD, EORT1, EORT2, EVARNTOP
c     .. EXECUTABLE STATEMENTS ...
c
      icall = icall + 1
      FVAL  = 0.0D0
...
c     ----------------------------------
c
      RS = 1.0D0   ! Natural Size of the System 
c
c     ------------------------------------------------------------------
c     ASSIGNING MINUIT VARIABLES ( VARIATIONAL PARAMETERS )
c
       z0          =  XV(1)
       alfa1       =  XV(2)
       alfa2       =  XV(3)
...
c
       ...
       ! Constricciones de normalizabilidad
       IF((alfa1+alfa2-alfac).LE.0)THEN
       FVAL=1000.0D0
       print*,'Rompe Normalizabilidad'       
       END IF
       IF ((alfa1+alfa2-alfac).LE.0) GOTO 215

c********Condicion de ortogonalidad************
...
c
                    ...
                    DO Iz = 1, nzN
                       DO Irho = 1, nrhoN
                             ...
                             EPS  = EPNUM(IREG)
c
c                      AORTO (SAME AS NUMERATOR) LIMITS (1)= rho, (2)= z
                      ANUM(1) = RLN(Irho)
                      BNUM(1) = RLN(Irho+1)
                      ANUM(2) = ZLN(Iz)*RS
                      BNUM(2) = ZLN (Iz+1)*RS
                      ...
                      IFAIL  = 1
                      FINV   = 0.0D0
      CALL D01FCF(NDIM,ANUM,BNUM,MINPTS,MAXPTS,EORT1,EPS,ACC,LENWRK,
     + WRKSTR,FINV,IFAIL)
                       AORTO1 = AORTO1+FINV
c  
                      ANUM(1) = RLN(Irho)
                      ...
                      IFAIL  = 1
                      FINV   = 0.0D0
      CALL D01FCF(NDIM,ANUM,BNUM,MINPTS,MAXPTS,EORT2,EPS,ACC,LENWRK,
     + WRKSTR,FINV,IFAIL)
                       AORTO2 = AORTO2+FINV
c
                       END DO
...
c
      ...
      write(*,*)
      write(*,*)
      write(*,*)'IREG,   XNU(IREG),    EPS,      ACC,      IFAIL'
      write(*,*)'------------------------------------------------'
      end if
c
c      write(*,*)'flag1',evarnutop
      enernutop= 0.0D0
      IREG  = 0
      DO Irho = 1,nrhoN   
         DO Iz = 1,nzN
...
c
c     Integra termino topologico
      CALL D01FCF(NDIM,ANUM,BNUM,MINPTS,MAXPTS,EVARNTOP,EPS,ACC,LENWRK,
     + WRKSTR,FINVNTOP,IFAIL)

      XNU(IREG)=FINVNTOP

c
      if(icorr.eq.1)then
      write(*,5004)IREG,FINVNTOP,EPS,ACC,IFAIL
      end if

 5004 format(1x,I4,2x,E12.6,2x,2(E8.3,2x),1x,I2)
c
      enernutop = enernutop + XNU(IREG)
...
c      write(*,*)'flag2',evarnutop
c
c                     *********************************
c                        VARIATIONAL ENERGY NUMERATOR
c                     *********************************
c
c
      if(icorr.eq.1)then
...
c
          ...
          FINVN  = 0.0D0
c
      CALL D01FCF(NDIM,ADEN,BDEN,MINPTS,MAXPTS,EVARN,EPS,ACC,LENWRK,
     + WRKSTR,FINVN,IFAIL)
      CALL D01FCF(NDIM,ADEN,BDEN,MINPTS,MAXPTS,EVARD,EPS,ACC,LENWRK,
     + WRKSTR,FINVD,IFAIL)
c
      XNU(IREG)=FINVN
      XDE(IREG)=FINVD
      if(icorr.eq.1)then
      write(*,5004)IREG,XDE(IREG),EPS,ACC,IFAIL
      end if
c
c
      enerde = enerde + XDE(IREG)
      enernu = enernu + XNU(IREG)
...
c
c     *************************
        FVAL = (1.0*enernutop+enernu)/enerde 
c     *************************
c     *************************
 215    if( FVAL .LE. ebest) then
           icallb = icall
           ebest  = FVAL
        end if
c
...
c
      ...
      write(*,*)
      write(*,*)
      write(*,*)' H like in a media        call: ',icall
      write(*,*)'****************************************************'
      write(*,*)'numerator(free+topologic): (',enernu,'+',enernutop,')'
      write(*,*)'denominator: ',enerde
      write(*,*)' Variational Energy      =',FVAL
      write(*,*)
      write(*,1010)' z0          =',  z0      
      write(*,1010)' alfa1       =',  alfa1     
      write(*,1010)' alfa2       =',  alfa2
      write(*,1010)' alfac       =',  alfac
      write(*,1010)' cc          =',  cc
c La dimensión se elige en el archivo de entrada
      write(*,*)
      write(*,*)'**********************************************'
      write(*,*)'              Ebest=',ebest,' call:',icallb
      write(*,*)
c
 1010 format(1x,1A12,1F18.14)
c
c                    *****************************
c                             CORRECTIONS 
c                    *****************************
      if(icorr.eq.1)then
c
         write(*,*)'Calculating Relative Corrections ...'
         write(*,*)'           ----------               '
         write(*,*)
c     1) DeltaRHO NUMER
          ANUM(1)   = RLN(nrhoN+1)  
          BNUM(1)   = 2.0D0*RLN(nrhoN+1)
          ANUM(2)   = ZLN(1)*RS  
          BNUM(2)   = ZLN(nzN+1)*RS
          ANUM(3)   = AKLN(1)
          BNUM(3)   = AKLN(nakN+1)
          ...
          IFAIL     = 1
          FINV      = 0.0D0
      CALL D01FCF(NDIM,ANUM,BNUM,MINPTS,MAXPTS,EVARN,EPS,ACC,LENWRK,
     + WRKSTR,FINV,IFAIL)
      write(*,*)
      write(*,*)'Corrections in  rho   EPS         ACC      (NUMER)    '
      write(*,5005)ABS(FINV/enernu),EPS,ACC
c
      write(*,*)' --------------------------------------------------'
c
c     2) DeltaRHO  DENOM
          ADEN(1)   =  RLD(nrhoD+1)  
          BDEN(1)   =  2.0D0*RLD(nrhoD+1)
          ADEN(2)   =  ZLD(1)*RS  
          BDEN(2)   =  ZLD(nZd+1)*RS
          ADEN(3)   =  AKLD(1)
          BDEN(3)   =  AKLD(nakD+1)
     ...
     + WRKSTR,FINV,IFAIL)
      write(*,*)
      write(*,*)'Corrections in  rho  EPS         ACC       (DENOM)    '
      write(*,5005)ABS(FINV/enerde),EPS,ACC
c
      write(*,*)' --------------------------------------------------'
c
c     3) DeltaZ+ NUMER
     ...
     + WRKSTR,FINV,IFAIL)
      write(*,*)
      write(*,*)'Corrections in  z+    EPS         ACC      (NUMER)    '
      write(*,5005)ABS(FINV/enernu),EPS,ACC
c
      write(*,*)' --------------------------------------------------'
c
c     4) DeltaZ+ DENOM
     ...
     + WRKSTR,FINV,IFAIL)
      write(*,*)
      write(*,*)'Corrections in  z+    EPS         ACC      (DENOM)    '
      write(*,5005)ABS(FINV/enerde),EPS,ACC
c
      write(*,*)' --------------------------------------------------'
c
...
c     3) Deltak + NUMER
     ...
     + WRKSTR,FINV,IFAIL)
      write(*,*)
      write(*,*)'Corrections in  k+    EPS         ACC      (NUMER)    '
      write(*,5005)ABS(FINV/enernu),EPS,ACC
c
      write(*,*)' --------------------------------------------------'
c
c     4) Deltak+ DENOM
     ...
     + WRKSTR,FINV,IFAIL)
      write(*,*)
      write(*,*)'Corrections in  k+    EPS         ACC      (DENOM)    '
      write(*,5005)ABS(FINV/enerde),EPS,ACC
      
      end if
c
c
 5005 format(4x,3(E12.6,2x))
c
c     ****************** END CORRECTIONS ***********************
c
c      print*,'flag',incharge
      RETURN
      END
c     ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c                             END FCN
...
c                           NUMERATOR BEGIN
c
c     SUBROUTINE EVARN(NDIM,XN)
c
c     ***************************************************************
c                            Yukawa Potential
c     ***************************************************************
c
c     This SUBROUTINE calculates the INTEGRAND of the ENERGY NUMERATOR 
c
c     NOTES
c
c     -- Integration cylindrical coordinates: rho_e and z_e   
c     -- Subroutine to be used by the NAG integration routine
c        D01FCF NAG-lib
c
c
      DOUBLE PRECISION FUNCTION EVARN(NDIM,XN)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c     ...ELECTRON COORDINATES (INTEGRATION VARIABLES)..
c     XN(1)=rhoe   XN(2)=z_e 
      DIMENSION XN(NDIM)
c     ...DISTANCES FROM ELECTRON TO EACH CENTER AND OTHER VARIABLES
c     ...COMMON BLOCKS  WAVE FUNCTIONS PARAMETERS
      COMMON / PARHAM /  ep1,ep2,efm,im,incharge,sigma,k 
      COMMON / PARM / B, AA, a, h, c, d, alpha, beta, delta
      COMMON / PARM2 / z0, alfac, alfa1, alfa2, cc 
      COMMON /FLAGS / iqflag,nmax
...
c     ELECTRON COORDINATES 
      rho   = XN(1)        ! This coordinate is sqrt(B)*rho_e
      z     = XN(2)
c
c
      r=sqrt(rho**2+z**2)
      r1  = sqrt(rho**2+(z+z0)**2)
      r2  = sqrt(rho**2+(z-z0)**2)
      
c     ---------------------------------------------------------------
c     Distances form the electron to the center: 
c     ---------------------------------------------------------------
c
      chi = (r1+r2)/(2.0d0*z0)
      eta = (r1-r2)/(2.0d0*z0)
...
c
c     Definitions:
c     phiN= 1.0d0*(alfa1*r1+ alfa2*r2 - 4.0d0*alfac*z0*chi*eta)
      phiN =  alfa1*r1+ alfa2*r2 - 4.0d0*alfac*z0*chi*eta

c
c      La dimensión del espacio se denota por delta
      psi0=(cc-r2)*eta**k*dexp(-phiN)

c     removing singularities
c      
        if(z0*chi*eta<XMIN)then
           EVARN=ZERO
           return
        endif
c        if(abs((chi**2-1)*(1-eta**2))<XMIN)then
c           EVARN=ZERO
c          write(6,*)'finish'           
c          return
c        endif
c        if(abs(chi**2-1.)<1D-9)then
c           EVARN=ZERO
c          write(6,*)'finish'
c          return
c        endif
...
c
c     ----------------------------------------------------
c     Potential associated with the trial wave function:
c     ----------------------------------------------------
c

         VVa =  (z0*((-chi + eta)*(-1 + eta**2)*k**2 - 
     &  k*(chi + 2*(alfa1 - alfa2 - 4*alfac*chi)*eta**4*z0 + 
     &  eta**2*(chi - 2*alfa1*z0 + 2*alfa2*z0 + 8*alfac*chi*z0) + 
     &  eta*(1 + 2*alfa1*chi*z0 - 2*alfa2*chi*z0 - 8*alfac*chi**2*z0) + 
     &  eta**3*(-3 - 2*alfa1*chi*z0 + 2*alfa2*chi*z0
     &  + 8*alfac*chi**2*z0)) + 
     &  eta**2*(alfa1**2*(-chi + eta)**2*(chi + eta)*z0**2 + 
     &  (chi + eta)*(2 + 4*alfa2*(-chi + eta)*z0
     &  + alfa2**2*(-chi + eta)**2*z0**2
     &  + 16*alfac**2*(-chi + eta)**2*z0**2 + 8*alfac*(-1
     &  + chi*eta)*z0*(1 + alfa2*(-chi + eta)*z0)) 
     &  - 2*alfa1*z0*(alfa2*(-(chi*(-2 + chi**2))
     &  + (-2 + chi**2)*eta - chi*eta**2 + eta**3)*z0  
     &  + 2*(-1 + chi**2 + 2*alfac*chi**2*z0 + 2*alfac*chi*eta**3*z0
     &  + eta**2*(1 + alfac*(2 - 4*chi**2)*z0) + chi*eta*(-1
     &  + 2*alfac*(-2 + chi**2)*z0))))) 
     &  + cc*((-1 + eta**2)*k**2 + k*(1 + eta**2 - 8*alfac*chi*eta*z0
     &  + 8*alfac*chi*eta**3*z0 - 2*alfa1*eta*(-1 + eta**2)*z0
     &  + 2*alfa2*eta*(-1 + eta**2)*z0)
     &  + eta**2*z0*(alfa1**2*(-chi**2 + eta**2)*z0 
     &  + (chi + eta)*(alfa2**2*(-chi + eta)*z0 + 16*alfac**2*(-chi
     &  + eta)*z0 + alfa2*(2 + 8*alfac*(-1 + chi*eta)*z0)) 
     &  - 2*alfa1*(alfa2*(-2 + chi**2 + eta**2)*z0 + (-chi
     &  + eta)*(1 + 4*alfac*(z0 + chi*eta*z0))))))/
     &  (eta**2*(-chi**2 + eta**2)*z0**2*(cc + (-chi + eta)*z0))

c$$$      VVa = ((-1 + eta**2)*k**2 + k*(1 + eta**2 + 2*(alfa1
c$$$     & - alfa2 - 4*alfac*chi)*eta*z0 + 
c$$$     &       2*(-alfa1 + alfa2 + 4*alfac*chi)*eta**3*z0) + 
c$$$     &    eta**2*z0*(alfa1**2*(-chi**2 + eta**2)*z0 + 
c$$$     &     2*alfa1*(chi - eta + 2*alfa2*z0 - 4*alfac*eta*z0
c$$$     &   - alfa2*eta**2*z0 - 
c$$$     &     chi**2*(alfa2 - 4*alfac*eta)*z0 - 4*alfac*chi*(-1
c$$$     &      + eta**2)*z0) + 
c$$$     &     (chi + eta)*(alfa2**2*(-chi + eta)*z0
c$$$     &      + 16*alfac**2*(-chi + eta)*z0 + 
c$$$     &     alfa2*(2 + 8*alfac*(-1 + chi*eta)*z0))))/(eta**2*(
c$$$     &   -chi**2 + eta**2)*z0**2)
      
      

      VVor= - (1/(ep2*z0))*(incharge/(chi-eta)+((ep2-ep1)/(ep2+ep1))*(       
     &     1.0d0*incharge/(abs(chi+eta)) - 1.0d0/(4*chi*eta)
     &     -incharge**2/2.0))
     &     + (1*im*im/(z0**2*(chi**2-1)*(1-eta**2)))/(efm)
      
 
      
      EVARN = psi0*(-VVa/(efm*2.0d0) + VVor )*psi0*rho
c      EVARN = psi0*psi0*rho
...
c
c
      DOUBLE PRECISION FUNCTION EVARNTOP(NDIM,XN)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c     ...ELECTRON COORDINATES (INTEGRATION VARIABLES)..
c     XN(1)=rhoe   XN(2)=z_e 
      DIMENSION XN(NDIM)
...
c
c     funcion de prueba
c      cc2=2.07814219400739
      psi0=(cc-r2)*eta**k*dexp(-phiN)

c     Definiciones utiles para termino topologico
      alpha=sqrt((ak/2.0)*(sqrt(ak**2+sigma**2)+ak))
      beta=sqrt((ak/2.0)*(sqrt(ak**2+sigma**2)-ak))
...
c        if(abs(z0**2*(chi**2-1)*(1-eta**2))>1.0D-3)then
c            EVARN=ZERO
c           write(6,*)'finish'           
c           return
c         endif
...
! calculamos la funcion de bessel en la variable correspondiente
!     y algunos factores correspondientes al termino topologico

        coef0=incharge*ep1/(ep1+ep2)
        var=z0*ak*Sqrt((chi**2-1.0)*(1-eta**2))    
        zbessel0=0.d0
        call Bessel(0,var,nmax,zbessel0) 
        expfac01=dexp(-z0*ak*(chi*eta+1.0))
        expfac02=dexp(-2.0*z0*ak*chi*eta)
       

!   primer termino topologico (termino 0)
        aint0 = coef0*(1.0/(ep1*(alpha**2+beta**2)+ep2*ak**2+
     &       (ep1+ep2)*ak*alpha))*(alpha**2+beta**2-ak**2)*
     &   (2.0*zbessel0*expfac01+expfac02)
c      aint0=expfac01
...
c      EVARN = psi0*(-VVa/(efm*2.0d0) + 1.0*aint0+ VVor )*psi0*rho
       EVARNTOP = psi0*(aint0)*psi0*rho
c     
c
      RETURN
      END
...
c     ***************************************************************
c
c     SUBROUTINE EVARD(NDIM,XD)
c     Time-stamp: <1999-08-12 11:32:35 vieyra> 
c
c     ***************************************************************
c                      Yukawa Potential
c     ***************************************************************
...
c     NOTES
c
c     -- Uses cylindrical coordinates   
c     -- Subroutine to be used by the NAG integration routine
c        D01FCF NAG-lib
c
      DOUBLE PRECISION FUNCTION EVARD(NDIM,XD)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c     ...ELECTRON COORDINATES (INTEGRATION VARIABLES)..
c     XN(1)=rho_e   XN(2)=z_e 
      DIMENSION XD(NDIM)
c     ...COMMON BLOCKS  WAVE FUNCTIONS PARAMETERS
      COMMON / PARHAM / ep1,ep2,efm,im,incharge,sigma,k
      COMMON / PARM / B, AA, a, h, c, d, alpha, beta, delta
...
c     Definitions:
      ...
      eta = (r1-r2)/(2.0d0*z0)
c     ----------------------------              
c      phiN = alfa1*r1 + alfa2*r2 - 4.0d0*alfac*z0*chi*eta
      phiN = alfa1*r1 + alfa2*r2 - 4.0d0*alfac*z0*chi*eta
c     
...
c
            ...
            DOUBLE PRECISION FUNCTION EORT1(NDIM,XD)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c     ...ELECTRON COORDINATES (INTEGRATION VARIABLES)..
c     XN(1)=rho_e   XN(2)=z_e 
      DIMENSION XD(NDIM)
...
c     
      ...
      psi0 = eta**k*dexp(-phiN)

      phiNgs = alfa1gs*r1 + alfa2gs*r2 - 4.0d0*alfacgs*z0*chi*eta
c
      psi0gs = eta**k*dexp(-phiNgs)
c        Energy Denominator
      EORT1 = r2*psi0*psi0gs*rho
c      EVARD=z0
      ...
      END
        ...
        DOUBLE PRECISION FUNCTION EORT2(NDIM,XD)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c     ...ELECTRON COORDINATES (INTEGRATION VARIABLES)..
c     XN(1)=rho_e   XN(2)=z_e 
      DIMENSION XD(NDIM)
...
c  ****************************************************************      
      SUBROUTINE Bessel(ia,x,nmax,z)
      IMPLICIT  REAL*8(A-H,O-Z)

      z=0q0;
      do m=0,nmax
         call factorial(m,fac)
         call factorial(m+ia,gamma)
         z=z+((-1)**m*(x/2.0)**(2*m+ia))/(fac*gamma)
 !        print*,m,((-1)**m*(x/2.0)**(2*m+ia))/(fac*gamma)
      enddo
...
c     Subroutine factorial

c***************************************************************      

      SUBROUTINE factorial(j,res)
      IMPLICIT  REAL*8(A-H,O-Z)
...
C      CALL MNINIT (JSYSRD,JSYSWR,JSYSSA)
...
c     IF(KM1)100,50,40
      IF(KM1.LT.0) then 
       GOTO 100
      ELSEIF (KM1.EQ.0)then 
       GOTO 50
      ELSEIF (KM1.GT.0)then 
       GOTO 40
      ENDIF
c
   40 DO 49 J=1,KM1
      ...
      Q(J)=A(J,K)*Q(K)
   49 A(J,K)=0.
c  50 IF(K-N)51,60,100
   50 IF((K-N).LT.0)then 
        GOTO 51
      ELSEIF((K-N).EQ.0)then 
        GOTO 60
      ELSEIF((K-N).GT.0)then 
        GOTO 100
      END IF
c
   51 DO 59 J=KP1,N
