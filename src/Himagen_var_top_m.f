...
c     .. COMMON BLOCK  FOR WAVE FUNCTIONS PARAMETERS ...
      ...
      COMMON / PARM2 / z0, alfac, alfa1, alfa2
...
c     .. EXTERNAL FUNCTIONS ...
      EXTERNAL          D01FCF, EVARN, EVARD, EVARNTOP
c     .. EXECUTABLE STATEMENTS ...
c
      icall = icall + 1
      FVAL  = 0.0D0
...
c
       ...
       IF((alfa1+alfa2-2.0*alfac).LE.0)THEN
       FVAL=1000.0D0
...
c
      if(icorr.eq.1)then
      write(*,5004)IREG,XNU(IREG),EPS,ACC,IFAIL
      end if
...
c$$$cc     In this section we calculate the quadrupole moment:
c$$$          iqflag = 1   ! flag to calculate Quadrupole Moment 
c$$$          MINPTS = 0
c$$$          IFAIL  = 1
c$$$          FINVQ  = 0.0D0
c$$$c
c$$$      CALL D01FCF(NDIM,ANUM,BNUM,MINPTS,MAXPTS,EVARN,EPS,ACC,LENWRK,
c$$$     + WRKSTR,FINVQ,IFAIL)
c$$$c
c$$$      quadm = quadm + FINVQ
c$$$c
c$$$          iqflag=0   ! back to numer
...
c
      ...
      END DO
c
c     END NUMERATOR CALCULATION 
c     ****************************************************************
c
c
c                     ***********************************
c                        VARIATIONAL ENERGY DENOMINATOR
c                     ***********************************
c
      if(icorr.eq.1)then
      write(*,*)
      write(*,*)'IREG,   XDE(IREG),    EPS,      ACC,      IFAIL'
      write(*,*)'------------------------------------------------'
      end if
c
c
...
c     phiN= 1.0d0*(alfa1*r1+ alfa2*r2 - 4.0d0*alfac*z0*chi*eta)
      ...
      facm = ((chi**2 - 1)*(1 - eta**2))**(im/2.0)
c
c     funcion de prueba
c     psi0=eta**k*dexp(-phiN)
      psi0=facm*eta**k*dexp(-phiN)
...
***************************
        coef1=2.0*incharge*ep1
        zbessel1=0.d0
        call Bessel(1,var,nmax,zbessel1) 
...
c      aint0=expfac01
        ...
        aint1 = coef1*beta*zbessel1*expfac01*(1/(ep1*(alpha**2
     &   +beta**2)+ep2*ak**2+(ep1+ep2)*ak*alpha))


      

c     EVARN = psi0*(-VVa/(efm*2.0d0) + 1.0*aint0+ VVor )*psi0*rho
       div=Sqrt((chi**2-1.0)*(1-eta**2))
       EVARNTOP = psi0*(aint0 + (im/(z0*efm))*(aint1/div) )*psi0*rho
c     
c
      RETURN
      END
...
c
        ...
        VVa  = ((-1 + chi**2)*(-1 + eta**2)**2*k**2 + 
     &       (-1 + chi**2)*(-1 + eta**2)*k*(1 + eta**2 + 4*eta**2*im
     &       - 8*alfac*chi*eta*z0 + 8*alfac*chi*eta**3*z0 - 
     &       2*alfa1*eta*(-1 + eta**2)*z0 + 2*alfa2*eta*(-1
     &   + eta**2)*z0) + 
     &       eta**2*(alfa1**2*(-1 + chi**2)*(-1 + eta**2)*(-chi**2
     &   + eta**2)*z0**2 + 
     &       (chi + eta)*(alfa2**2*(-1 + chi**2)*(-chi + eta)*(-1
     &   + eta**2)*z0**2 + 
     &   2*alfa2*(-1 + chi**2)*(-1 + eta**2)*z0*(1 + 2*im + 4*alfac*(-1
     &   + chi*eta)*z0) - 
     &       4*(-chi + eta)*(im**2 - 4*alfac**2*(-1 + chi**2)*(-1
     &   + eta**2)*z0**2)) - 
     &   2*alfa1*(-1 + chi**2)*(-1 + eta**2)*z0*
     &       (alfa2*(-2 + chi**2 + eta**2)*z0 +
     &  (-chi + eta)*(1 + 2*im + 4*alfac*(z0 + chi*eta*z0)))))/
     &  ((-1 + chi**2)*eta**2*(-1 + eta**2)*(-chi**2 + eta**2)*z0**2)
     ...
     &     + (1*im*im/(z0**2*(chi**2-1)*(1-eta**2)))/(efm)
      ...
      EVARN = psi0*(-1.0*VVa/(efm*2.0d0) + 1.0*VVor )*psi0*rho
